...
ds <- read_csv('data/imputed-gain-data.csv',
               col_types = cols(gender = col_factor(),
                                race = col_factor(),
                                codis4 = col_factor())) %>%
  mutate(codis4 = case_when(codis4_n == 1 ~ "Neither",
                            codis4_n == 2 ~ "Both",
                            codis4_n == 3 ~ "Externalizing Only",
                            codis4_n ==  4 ~ "Internalizing Only",
                            TRUE ~ NA_character_)) %>%
  mutate(codis4 = factor(codis4, levels = c("Neither", "Internalizing Only", 
...
ds_syn = ds_1_syn$syn %>%
  select(acra_derived, tss_0, tss_3, tss_6, sfs8p_0, sfs8p_3, sfs8p_6,
         ...
         sncnt, engage) %>%
  rename(treat = acra_derived,
         nproc = aes_c) %>%
  mutate(treat = case_when(treat == 1 ~ "A",
                           treat == 0 ~ "B",
                           TRUE ~ NA_character_))
ds_1 = ds_1 %>%
  select(acra_derived, tss_0, tss_3, tss_6, sfs8p_0, sfs8p_3, sfs8p_6,
